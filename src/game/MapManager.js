export const REGIONS = Object.freeze([
  { id: 'sect_hall', name: 'Sảnh Điện Tu Tiên', level: 'Luyện Khí Cấp 1', requiredOrder: 0, requiredStage: 1, mapOrder: 0, portal: { x: 0, z: 26 }, townGate: { x: 0, z: 26 }, marker: { x: 45, y: 45 } },
  { id: 'luoyang', name: 'Lạc Dương Thành', level: 'Trúc Cơ Cấp 1', requiredOrder: 1, requiredStage: 1, mapOrder: 1, portal: { x: 28, z: -8 }, townGate: { x: 25, z: -5 }, marker: { x: 67, y: 36 } },
  { id: 'spirit_mine', name: 'Mỏ Linh Thạch', level: 'Trúc Cơ Cấp 3', requiredOrder: 1, requiredStage: 3, mapOrder: 2, portal: { x: 18, z: 34 }, townGate: { x: 16, z: 31 }, marker: { x: 61, y: 77 } },
  { id: 'heaven_sect', name: 'Thiên Vân Môn', level: 'Kim Đan Cấp 1', requiredOrder: 2, requiredStage: 1, mapOrder: 3, portal: { x: -30, z: -18 }, townGate: { x: -27, z: -16 }, marker: { x: 26, y: 31 } },
]);

export class MapManager {
  constructor({ overlay, realmOrder = 0, subStage = 1, currentRegion = 'sect_hall', onTeleport, onClose }) {
    this.overlay = overlay; this.realmOrder = realmOrder; this.subStage = subStage; this.currentRegion = currentRegion;
  ...
  isUnlocked(region) {
    return this.realmOrder > region.requiredOrder
      || (this.realmOrder === region.requiredOrder && this.subStage >= region.requiredStage);
  }
  ...
  setCultivation(realmOrder, subStage) {
    this.realmOrder = Math.max(0, Number(realmOrder) || 0);
    this.subStage = Math.max(1, Number(subStage) || 1);
    this.bindNodes();
  }
