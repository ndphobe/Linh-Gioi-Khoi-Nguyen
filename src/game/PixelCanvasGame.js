...
export class CultivationGame {
  ...
  mergeSelf(data = {}) {
    const oldHp=this.player.hp,nextHp=Number(data.hp??oldHp),serverTotalAtk=Number(data.totalAtk);this.profile.maxHp=Number(data.maxHp??this.profile.maxHp);this.profile.maxMp=Number(data.maxMp??this.profile.maxMp);this.profile.mp=Number(data.mp??this.profile.mp);this.profile.qi=Number(data.qi??this.profile.qi);if(Number.isFinite(Number(data.basicDamage)))this.profile.basicDamage=Number(data.basicDamage);
  ...
  ensureAttackStat(){const tags=document.querySelector('.player-panel .player-tags');if(!tags||document.getElementById('attack-power'))return;const el=document.createElement('span');el.className='attack-power-tag';el.innerHTML='Công <b id="attack-power">0</b> · ST thường <b id="basic-damage">0</b>';tags.appendChild(el);this.ui.attackPower=el.querySelector('#attack-power');this.ui.basicDamage=el.querySelector('#basic-damage');}
