...
export class CultivationGame {
  ...
  receivePlayerDamage(amount,nextHp=this.player.hp){const hit=this.player.syncServerDamage(nextHp,amount);this.damageNumbers.push({x:this.player.position.x,z:this.player.position.z,value:`-${Math.round(hit.damage)}`,player:true,life:.9,max:.9});this.animationController.play('hurt',{},true);this.ui.app?.classList.remove('damage-flash');void this.ui.app?.offsetWidth;this.ui.app?.classList.add('damage-flash');setTimeout(()=>this.ui.app?.classList.remove('damage-flash'),300);this.audio?.play('hurt');}
  ...
  destroy(){this.state.running=false;if(this.frameRequest)cancelAnimationFrame(this.frameRequest);this.mapManager?.destroy();this.goldDropSystem?.clear();this.vfxManager?.clear();this.uiManager?.destroy();this.skillTreePanel?.destroy();for(const overlay of [this.shopUI,this.inventoryUI,this.tribulationUI])overlay?.remove();window.removeEventListener('keydown',this.bound.keydown);window.removeEventListener('keyup',this.bound.keyup);window.removeEventListener('resize',this.bound.resize);this.socket?.disconnect();}
}
