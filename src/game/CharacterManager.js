...
export class CharacterManager{
  ...
  create(faction,name='Vô Danh'){
    ...
    if(!this.characters[id])this.characters[id]={id,faction,name,realm:'qi_refining',realmName:'Luyện Khí Tầng 1',minorLevel:1,currentExp:0,maxExp:100,gold:0,inventory:[],equipment:{weapon:null,armor:null,accessory:null},allocatedStats:{strength:0,spirit:0,vitality:0,agility:0},skillSystem:null,currentRegion:'sect_hall'};
  ...
  updateActive(profile){
    ...
    const realm=profile.realm??current.realm,minorLevel=profile.skillSystem?.minorLevel??profile.minorLevel??current.minorLevel;
    Object.assign(current,{name:profile.name??current.name,faction:profile.faction??current.faction,realm,realmName:realmTitle(realm,minorLevel),minorLevel,currentExp:profile.skillSystem?.cultivationProgress??profile.qi??current.currentExp,maxExp:100,gold:profile.shopSystem?.gold??profile.gold??current.gold,inventory:[...(profile.shopSystem?.inventory??current.inventory)],equipment:{...current.equipment,...profile.shopSystem?.equipment},allocatedStats:{...current.allocatedStats,...profile.allocatedStats},skillSystem:profile.skillSystem??current.skillSystem,currentRegion:profile.currentRegion??current.currentRegion});
